...
turbohtml covers the same pipeline in a C extension. Parsing is unsanitized by default; use
:func:`turbohtml.clean.sanitize` at an untrusted-HTML boundary. That explicit split matters during migration because
``JustHTML(source)`` and :func:`turbohtml.parse` do not have the same security contract.
...
.. list-table::
    ...
    - - Published WPT comparison
      - `1,782/1,880 (94.8%) <https://emilstenstrom.github.io/justhtml/comparison.html>`__ for turbohtml 1.4.0 after
        `JustHTML #80 <https://github.com/EmilStenstrom/justhtml/pull/80>`__ corrected 11 adapter-only mismatches.
        Current source passes 1,880/1,880 after eight case-specific corrections where the fixtures conflict with the
        living standard; the raw fixture result is 1,872/1,880. `Issue #719
        <https://github.com/tox-dev/turbohtml/issues/719>`__ records the corpus and corrections.
...
<https://github.com/EmilStenstrom/justhtml/blob/main/benchmarks/correctness.py>`__ where the APIs overlap.
...
.. list-table::
    ...
    - - ``JustHTML(source)``
      - :func:`turbohtml.clean.sanitize` when the result is HTML, or sanitize then :func:`turbohtml.parse` when a tree
        is required
...
- :func:`turbohtml.parse` does not sanitize. Replace a default ``JustHTML(source)`` with
  :func:`turbohtml.clean.sanitize`, not with a bare parse, when ``source`` is untrusted.
- JustHTML's selector engine does not accept ``:has()``. A selector kept deliberately within that subset ports as-is;
  turbohtml can simplify Python-side parent filtering by moving it into ``:has()``.

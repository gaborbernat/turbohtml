...
Layout-aware text carries entry and exit state for lists, tables, and annotations. A tree nested 1,024 levels or deeper
raises :exc:`RecursionError` before rendering; :attr:`~turbohtml.Node.text`, which only concatenates text descendants,
has no nesting limit.

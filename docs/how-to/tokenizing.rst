...
resolved (so ``handle_entityref``/``handle_charref`` never fire), and a CDATA section reaches ``handle_comment`` rather
than ``unknown_decl``. Processing instructions reach ``handle_pi``; the reserved ``xml`` and ``xml-stylesheet`` targets
remain comments.

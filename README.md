...
[markupsafe](https://markupsafe.palletsprojects.com)-compatible `turbohtml.migration.markupsafe` covers template autoescaping, and
...
For template output, `turbohtml.migration.markupsafe` is a markupsafe drop-in: `Markup` marks trusted HTML, and combining it with
untrusted values escapes them. Swap `from markupsafe import ...` for `from turbohtml.migration.markupsafe import ...`:
...
- `turbohtml.migration.markupsafe.escape` matches markupsafe and runs 2–3× faster on the small strings template autoescaping escapes.

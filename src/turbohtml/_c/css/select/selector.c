...
/* Whether any element in the subtree below node is the subject of rel anchored at anchor. */
static int sel_has_subtree(th_node *node, const sel_complex *rel, int subject, th_node *anchor, const sel_ctx *ctx) {
    ...
    while (current != NULL) {
        ...
        while (current != node && sel_next_element_sibling(current) == NULL) {
            current = current->parent;
        }
        current = current == node ? NULL : sel_next_element_sibling(current);
    }
    return 0;
}

...
static th_node *copy_node_in_tree(th_tree *tree, th_node *source) {
    ...
    if (copy != NULL) { /* GCOVR_EXCL_BR_LINE: NULL only on alloc failure */
        ...
        int carries_attrs = source->type == TH_NODE_PI;
        if (!carries_attrs) {
            /* GCOVR_EXCL_BR_START: HTML cannot nest a doctype in selected content */
            carries_attrs = source->type == TH_NODE_DOCTYPE;
            /* GCOVR_EXCL_BR_STOP */
        }
        if (carries_attrs) {
...
static enum th_drain drain_in_body(th_tree *tree, th_token *tok, th_insert *dc) {
    ...
    if (tok->kind == TH_START_TAG) {
        ...
        if (atom == TH_TAG_INPUT) {
            /* an input closes an open select entirely (and is ignored outright
               in a select-context fragment) */
            if (tree->fragment_root != NULL && tree->ctx_atom == TH_TAG_SELECT) {
                return TH_DRAIN_NEXT;
            }
            if (has_in_scope(tree, TH_TAG_SELECT)) {

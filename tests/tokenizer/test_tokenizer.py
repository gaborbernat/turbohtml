...
def test_processing_instruction_token(document: str, target: str, data: str, width_prefix: str) -> None:
    ...
    assert token.type is TokenType.PROCESSING_INSTRUCTION
    assert token.target == target
    assert token.data == data
    assert token.source == document
    assert token.tag is None
...
def test_token_repr() -> None:
    ...
    assert repr(tokens[5]) == "Token(PROCESSING_INSTRUCTION, target='pi', data='data')"

...
def test_deep_text_and_serialization_are_complete(source: str) -> None:
    ...
    assert root.html.count("<x>") == _DEEP + (source == "mutation")
    assert "bottom" in root.serialize(Html(layout=Indent(1)))
...
def test_range_rejects_a_deep_partial_boundary_before_mutation(method: str, deep_boundary: str) -> None:
    ...
    assert root.html == before
    assert selection.start_container == (text_node if deep_boundary == "start" else root)
    assert selection.end_container == (root if deep_boundary == "start" else text_node)

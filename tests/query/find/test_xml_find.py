...
def test_xml_find_all_matches_exact_tag_name(tag: str, expected: list[str]) -> None:
    document = parse_xml("<Root><div/><DIV/><custom/><Custom/></Root>")
    assert [element.tag for element in document.find_all(tag)] == expected


@pytest.mark.parametrize("tag", ["div", "DIV", "custom", "Custom"])
def test_xml_find_matches_exact_tag_name(tag: str) -> None:
    document = parse_xml("<Root><div/><DIV/><custom/><Custom/></Root>")
    assert (match := document.find(tag)) is not None
